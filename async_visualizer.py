...
class AsyncVisualizer:
    ...
    def extract_timestamps(self, metric_key):
        timestamps = [
            ts for ts, _ in self.tracker.history.metrics_centralized[metric_key]
        ]
    ...
    def plot_final_centralized_confusion_matrix(self, folder_name: str):
        ...
        sns.heatmap(
            self.tracker.history.metrics_centralized["confusion_matrix"][-1][1],
            annot=True,
            fmt="d",
            ax=ax,
            xticklabels=nice_goal_label_names,
            yticklabels=nice_goal_label_names,
        )
        ax.set_title("Final Confusion Matrix")
        plt.savefig(
            "results/" + folder_name + "/final_centralized_confusion_matrix.png"
        )
        plt.clf()
    ...
    def make_centralized_metrics_plot(self, folder_name: str):
        ...
        timestamps = self.extract_timestamps(
            "accuracy"
        )  # Assuming all metrics share the same timestamps
        ...
        for ax, (metric_key, metric_name) in zip(axs, metrics):
            metric_values = extract_vals_from_metrics(
                self.tracker.history.metrics_centralized.get(metric_key, [])
            )
            self.plot_metric(
                ax,
                timestamps,
                metric_values,
                f"Centralized {metric_name}",
                "Timestamp",
                metric_name,
            )

        fig.suptitle(
            f"Centralized Metrics, asynchronous setting:\n{self.tracker.dataset_name}, train_time: {self.tracker.total_train_time},"
    ...
    def make_centralized_final_perclass_metrics_plot(self, folder_name: str):
        ...
        axs[0].bar(
            x_axis,
            self.tracker.history.metrics_centralized["precision_perclass"][-1][1],
        )
        axs[0].set_title("Final Precision per class")
        axs[0].set_xlabel("Class")
        axs[0].set_ylabel("Precision")
        axs[1].bar(
            x_axis, self.tracker.history.metrics_centralized["recall_perclass"][-1][1]
        )
        axs[1].set_title("Final Recall per class")
        axs[1].set_xlabel("Class")
        axs[1].set_ylabel("Recall")
        axs[2].bar(
            x_axis, self.tracker.history.metrics_centralized["f1_perclass"][-1][1]
        )
        axs[2].set_title("Final F1 per class")
        axs[2].set_xlabel("Class")
        axs[2].set_ylabel("F1")
        fig.suptitle(
            f"Final Metrics for setting:\n{self.tracker.dataset_name}, train_time: {self.tracker.total_train_time},\n"
        ...
        plt.savefig(
            "results/" + folder_name + "/centralized_final_perclass_metrics.png"
        )
        plt.clf()
    ...
    def make_target_counts_plot(self, folder_name: str):
        ...
        ax.set_title(
            f"Target counts for setting:\n{self.tracker.dataset_name},  train_time: {self.tracker.total_train_time},\ndata augmentation:{self.tracker.data_augmentation}"
            + f"async_aggregation_strategy:{self.tracker.async_aggregation_strategy}, mixing alpha:{self.tracker.fedasync_mixing_alpha}"
        )
        ax.set_xlabel("Client")
        ax.set_ylabel("Count")
        lgd = ax.legend(
            title="Class", bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0
        )

        plt.savefig(
            "results/" + folder_name + "/target_counts.png",
    ...
    def get_client_metrics(self, metric_name, client_id):
        ...
        if (
            str(client_id)
            in self.tracker.history.metrics_distributed_fit_async[metric_name]
        ):
            timestamps = [
                ts
                for ts, _ in self.tracker.history.metrics_distributed_fit_async[
                    metric_name
                ][str(client_id)]
            ]
            first_ts = timestamps[0]
            ...
            values = [
                val
                for _, val in self.tracker.history.metrics_distributed_fit_async[
                    metric_name
                ][str(client_id)]
            ]
    ...
    def count_times_started(self, metric_name):
        ...
        for i in range(self.tracker.num_clients):
            if (
                ...
                times_started[i] = len(
                    self.tracker.history.metrics_distributed_fit_async[metric_name][
                        str(i)
                    ]
                )
        return times_started

    def make_metrics_per_client_over_time_plot(
        self, folder_name: str, metric_name: str, title_suffix: str = ""
    ):
        ...
        fig.suptitle(
            f"{metric_name.capitalize()} per client over time (before training) for setting:\n{self.tracker.dataset_name}, train_time: {self.tracker.total_train_time},\nasync_aggregation_strategy:{self.tracker.async_aggregation_strategy}"
            + title_suffix
        )
        axs = axs.flatten()
        ...
        fig.savefig(
            f"results/{folder_name}/{metric_name.lower()}_per_client_over_time{title_suffix}.png"
        )
        plt.clf()

    def make_interval_plot_async(self, folder_name: str):
        times_started = self.tracker.history.metrics_distributed_fit_async[
            "time_started"
        ]
        local_train_times = self.tracker.history.metrics_distributed_fit_async[
            "local_train_time"
        ]

        start_timestamp = self.tracker.history.metrics_centralized["start_timestamp"][
            0
        ][1]
                ...
                new_df = pd.DataFrame(
                    [
                        [
                            ...
                            start_time + local_train_time - start_timestamp,
                        ]
                    ],
        ...
        for _, row in df.iterrows():
            ...
            ax.plot(
                [start, end], [client_positions[cid], client_positions[cid]], marker="o"
            )
    ...
    def make_f1_over_time_heatmap(self, folder_name: str):
        if getattr(self.tracker, "id", None) is not None:
            ...
            f1_scores = np.array(
                [
                    np.array(np.load(file, allow_pickle=True), dtype=np.float32)
    ...
    def make_f1_for_running_class_and_other_classes(self, folder_name: str):
        f1_pc = np.array(
            [
                np.array(f1s)
                for ts, f1s in self.tracker.history.metrics_centralized["f1_perclass"]
            ]
        )
        sns.heatmap(f1_pc)
...
def get_label_from_varied_params(varied_params: Dict[str, str], tracker: Tracker):
    if len(varied_params) > 4:
        ...
        res = ", ".join(
            [
                f"{varied_params[param]}={getattr(tracker, param)}"
                for param in list(varied_params.keys())[:half]
            ]
        )
        res += "\n"
        res += ", ".join(
        ...
        res = ", ".join(
            [
                f"{varied_params[param]}={getattr(tracker, param)}"
                for param in varied_params.keys()
            ]
        )
    # if 'iid' in res:
